...
\begin{abstract}
Robot stacks are often organized hierarchically, but their failure handling is not. A local fault can leak upward through perception, planning, and execution layers until the whole mission is degraded or the system enters a brittle global fallback. We argue that the central design problem is not only detecting or repairing faults, but containing their propagation across hierarchy levels. We propose \sys{}, a containment-budget view of robot hierarchies in which each level is allowed only a bounded amount of local retry and a bounded summary of failure state before escalation. The goal is to minimize failure blast radius, not merely maximize eventual success. A simple simulator shows that explicit containment budgets reduce average propagation depth relative to unconstrained escalation, while preserving high success in the tested regime. We position the idea against robot fault diagnosis, fault-tolerant control, runtime assurance, behavior-tree recovery, and classical fault containment theory, and we identify the robotics-specific assumptions that those lines of work usually leave implicit.
\end{abstract}
...
The directional result is simple: modest local retry already cuts propagation sharply. The important point is not that retries are always good. The point is that containment is measurable and controllable, and thus can be designed rather than hoped for.
...
The idea is weaker if the robot stack is flat, if the hierarchy is purely decorative, or if the environment is so benign that failure escalation never matters. It is also weaker if the evaluation never leaves a toy simulator. Those are real limitations.
